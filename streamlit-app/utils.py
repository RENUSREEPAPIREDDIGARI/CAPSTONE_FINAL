...
SYSTEM_PROMPT = """You are a professional social media content creator and analyst. 
Your task is to analyze images and generate engaging content or provide insights about social media posts.
Be concise, creative, and professional in your responses."""
...
def generate_image(model_id, body):
    ...
    try:
        ...
        if not response or 'body' not in response:
            logger.error("Invalid response from Bedrock API")
            return None
            
        response_body = json.loads(response.get("body").read())
        
        if not response_body:
            logger.error("Empty response body from Bedrock API")
            return None
...
def parse_task_response(response:str)->str:
    ...
    try:
        ...
        if not task1_response or not task2_response:
            logger.warning("Task responses not found in response")
            return ""
...
def get_task2_response(response:str)->str:
    ...
    try:
        ...
        if not task2_response:
            logger.warning("Task2 response not found")
            return ""
        return task2_response[0]
    except Exception as e:
        logger.error(f"Error parsing task2 response: {str(e)}")
        return ""
    ...
    for post_id in post_ids:
        ...
        try:
            ...
            if not post_response:
                logger.warning(f"No response found for post {post_id}")
                res.append("")
            else:
                res.append(post_response[0])
        except Exception as e:
            logger.error(f"Error parsing response for {post_id}: {str(e)}")
            res.append("")
    return res
