...
def find_similar_posts(product_type, tone, n=3):
    ...
    try:
        ...
        product_posts = df[df['file_name'].str.contains(product_type, case=False)]
        ...
        product_posts['similarity_score'] = product_posts.apply(calculate_similarity_score, axis=1)
        
        # Sort by similarity score and return top n posts
        similar_posts = product_posts.nlargest(n, 'similarity_score')
        # Include all necessary columns including the image path (5th column)
        return similar_posts[['text', 'file_name', 'local_path', 's3_path']].to_dict('records')
    except Exception as e:
...
def load_image_from_path(post):
    ...
    try:
        ...
        if 'local_path' in post and post['local_path'] and os.path.exists(post['local_path']):
            return Image.open(post['local_path'])
        # Then try the s3 path (which might be stored locally)
        elif 's3_path' in post and post['s3_path']:
            # Remove the s3:// prefix if present and get the file name
            image_name = post['s3_path'].split('/')[-1]
            # Check in the sample_images directory
        ...
        if st.button("Generate Content"):
            with st.spinner("Analyzing and generating content..."):
                ...
                if similar_posts:
                    ...
                    new_content = generate_content_from_patterns(similar_posts, product, brand, tone, description)
                    if new_content:
                        # Make content selectable and copyable
                        st.text_area("New Post", value=new_content, height=150, key="new_post", disabled=False, help="Click to select and copy the generated content")
